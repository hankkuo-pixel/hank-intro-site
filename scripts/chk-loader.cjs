...
;(async()=>{
 ...
 p.on('requestfailed',r=>fails.push('FAIL '+r.url().replace('http://localhost:8092','')+' :: '+((r.failure()||{}).errorText)))
 p.on('response',r=>{if(r.status()>=400)fails.push(r.status()+' '+r.url().replace('http://localhost:8092',''))})
 await p.goto('http://localhost:8092/',{waitUntil:'domcontentloaded'})
